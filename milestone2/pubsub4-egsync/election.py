...
def election(cfg, log) -> int:
    try:
        ...
        # working:
        time.sleep(3); build_manifest(cfg, log)        ; attack_all(cfg, log, 'build_manifest')
        time.sleep(3); mint_guardian_channels(cfg, log)
        time.sleep(3); mint_verifier_channels(cfg, log)
        time.sleep(3); announce_key_ceremony(cfg, log) ; attack_all(cfg, log, 'announce_key_ceremony')
        ...
        time.sleep(3); publish_joint_key(cfg, log)     ; attack_all(cfg, log, 'publish_joint_key')
        time.sleep(3); build_election(cfg, log)        ; attack_all(cfg, log, 'build_election')
        time.sleep(3); mint_device_channels(cfg, log) # TODO should this happen earlier?
        time.sleep(3); add_devices(cfg, log)           ; attack_all(cfg, log, 'add_devices')
        time.sleep(3); ids = vote_commit_all(cfg, log) ; attack_all(cfg, log, 'vote_commit_all')
        time.sleep(3); vote_reveal_all(cfg, log, ids)  ; attack_all(cfg, log, 'vote_reveal_all')
        time.sleep(3); tally(cfg, log)                 ; attack_all(cfg, log, 'tally')
        time.sleep(3); decrypt_shares(cfg, log)        ; attack_all(cfg, log, 'decrypt_shares')
        time.sleep(3); decrypt_results(cfg, log)       ; attack_all(cfg, log, 'decrypt_results')
